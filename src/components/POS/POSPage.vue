<template>
  <v-container fluid>
    <v-row>
      <v-col md="8">
        <v-form class="search-wrap" style="float: right">
          <v-row align="center">
            <v-col cols="9">
              <v-text-field
        ...
        <v-tabs background-color="primary" dark>
          <v-tab v-for="item in items" :key="item.label" :href="item.href">
            {{ item.label }}
          </v-tab>
        </v-tabs>
        <v-row>
          <v-col md="3" v-for="product in allproduct" :key="product.id">
  ...
  computed: {
    ...mapState({
     //allproduct: state => state.products.allproduct,
      //allcartproduct: state => state.cart.allcartproduct,
    }),
    ...mapGetters({
      sum: 'cart/sum',
    }),
